<?php $title=setting('brand_name','Bon de ordine').' · Status coada'; $publicTheme=true; require __DIR__.'/_head.php';
$logo = setting('brand_logo','');
$mmss = function($s){ $s=(int)$s; return $s>0 ? sprintf('%d:%02d', intdiv($s,60), $s%60) : ''; };
?>
<body class="statuspage">
<style>
/* culori din variabile => se aseaza corect si pe tema deschisa, si pe cea inchisa (comutator) */
.statuspage{background:var(--bg);color:var(--ink);min-height:100vh;margin:0}
.statuspage h1{color:var(--ink)}
...
.stbranch{color:var(--muted);font-weight:600}
.stgrid{display:grid;grid-template-columns:1fr;gap:1.2rem}
@media(min-width:720px){.stgrid{grid-template-columns:1.3fr 1fr}}
.stcard{background:var(--surface);border:1px solid var(--line);border-radius:16px;padding:1.1rem 1.2rem}
.stcard h2{margin:0 0 .8rem;font-size:.95rem;text-transform:uppercase;letter-spacing:.06em;color:var(--muted)}
.ctr-row{display:flex;align-items:center;gap:.8rem;padding:.6rem 0;border-bottom:1px solid var(--line)}
.ctr-row:last-child{border-bottom:none}
.ctr-code{font-weight:800;min-width:64px;color:var(--ink)}
.ctr-row .stsvc{color:var(--muted)}
.ctr-now{margin-left:auto;font-size:1.6rem;font-weight:800;letter-spacing:.02em;color:var(--accent)}
.ctr-free{margin-left:auto;color:var(--muted);font-weight:600}
.ctr-pause{margin-left:auto;color:var(--warn);font-weight:700;font-size:.85rem}
.wait-row{display:flex;align-items:center;gap:.7rem;padding:.5rem 0;border-bottom:1px solid var(--line)}
...
.wait-row .stwait{color:var(--muted)}
.stfoot{margin-top:1.4rem;color:var(--muted);font-size:.8rem;text-align:center}
.stdot{display:inline-block;width:8px;height:8px;border-radius:999px;background:var(--ok);margin-right:.3rem;animation:stpulse 2s infinite}
...
<div class="stwrap">
  <div class="sthead">
    ...
    <span style="margin-left:auto;font-size:.8rem;color:var(--muted)"><span class="stdot"></span>live</span>
  </div>
      ...
      <div id="ctrList">
        <?php foreach($counters as $c): ?>
          <div class="ctr-row">
            ...
            <span style="color:var(--muted)"><?= e($c['name']) ?></span>
        ...
        <?php endforeach; ?>
        <?php if(!$counters): ?><div style="color:var(--muted)">Niciun ghiseu.</div><?php endif; ?>
      </div>
    </div>
    <div class="stcard">
      <h2>La rand</h2>
      <div id="waitList">
        ...
        <?php foreach($waiting as $w): if((int)$w['cnt']<=0) continue; ?>
          <div class="wait-row">
            ...
            <span style="flex:1"><?= e($w['name']) ?><?php if(!empty($w['est'])): ?><br><span class="muted" style="font-size:.78rem;color:var(--muted)"><?= e($estMin($w['est'])) ?> așteptare</span><?php endif; ?></span>
            <span class="wait-cnt"><?= (int)$w['cnt'] ?></span>
          </div>
        <?php endforeach; ?>
        <?php if(!array_filter($waiting, fn($w)=>(int)$w['cnt']>0)): ?><div id="waitEmpty" style="color:var(--muted)">Nicio coada de asteptare.</div><?php endif; ?>
      </div>
    </div>
  </div>
...
(function(){
  ...
  async function refresh(){
    ...
    if(cl){ cl.innerHTML=(r.counters||[]).map(function(c){
      ...
      return '<div class="ctr-row"><span class="ctr-code">'+esc(c.code)+'</span><span style="color:var(--muted)">'+esc(c.name)+'</span>'+right+'</div>';
    }).join('')||'<div style="color:var(--muted)">Niciun ghiseu.</div>'; }
    ...
    if(wl){ var rows=(r.waiting||[]).filter(function(w){return (+w.cnt)>0;});
      ...
      wl.innerHTML=rows.map(function(w){ var e2=em(w.est);
        return '<div class="wait-row"><span class="wait-tag" style="background:'+esc(w.color)+'">'+esc(w.prefix)+'</span>'
          +'<span style="flex:1">'+esc(w.name)+(e2?'<br><span class="muted" style="font-size:.78rem;color:var(--muted)">'+e2+' așteptare</span>':'')+'</span>'
          +'<span class="wait-cnt">'+(+w.cnt)+'</span></div>';
      }).join('')||'<div style="color:var(--muted)">Nicio coada de asteptare.</div>'; }
  }
