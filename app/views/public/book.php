    ...
    <div class="card pad">
      ...
      <?php else: ?>
        ...
        <form method="post" action="<?= e(url('book/'.$svc['id']).$lq) ?>" id="bookForm" style="display:none;margin-top:1.2rem">
          ...
          <label style="display:flex;gap:.5rem;align-items:flex-start;font-size:.84rem;text-align:left;margin:.2rem 0 .9rem"><input type="checkbox" name="consent" value="1" required style="width:auto;margin-top:.2rem"><span><?= e($L['consent']) ?> <a href="<?= e(url('legal/privacy').$lq) ?>" target="_blank" rel="noopener"><?= e($L['privacy']) ?></a>.</span></label>
