<?php
/* Editor canvas pentru DISPENSER (acelasi motor ca afisorul TV, cu widget interactiv „Grila servicii").
   Layoutul canvas se salveaza in devices.config -> cheia „canvas" (fara sa atinga config-ul editorului clasic). */
$cfg = $dev['config'] ? json_decode($dev['config'], true) : null;
$canvas = (is_array($cfg) && isset($cfg['canvas'])) ? $cfg['canvas'] : null;
$accent = setting('accent_color', '#2563eb');
?>
<!doctype html><html lang="ro"><head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Editor canvas dispenser · <?= e($dev['name']) ?></title>
...
window.PLAYER = {
  ...
  config: <?= $canvas ? json_encode($canvas, JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_UNESCAPED_UNICODE) : 'null' ?>
};
</script>
