...
<div class="topbar"><h1>Bilete</h1>
  <form method="post" action="<?= e(url('backoffice/tickets/reset')) ?>" data-confirm="Resetezi bonurile? Se STERG TOATE biletele (coada + istoric + statistici) si numerotarea reincepe de la 0. Actiunea NU poate fi anulata." style="display:flex;gap:.4rem;align-items:center">
    <?= csrf_field() ?>
...
<p class="muted" style="margin-top:-.6rem;font-size:.82rem">„Reset bonuri" șterge <strong>doar biletele</strong> și repornește numerotarea de la 0 (fără backup). Pentru curățarea completă a datelor de test înainte de predare (bilete + programări + feedback + sesiuni, cu backup automat) folosește <a href="<?= e(url('backoffice/settings')) ?>">Setări → „⚠ Pregătire pentru producție"</a>.</p>
<form method="get" id="tkFilter" action="<?= e(url('backoffice/tickets')) ?>" class="card pad" style="display:flex;gap:.8rem;flex-wrap:wrap;align-items:flex-end;margin-bottom:1.2rem">
  ...
  <button class="btn" formaction="<?= e(url('backoffice/tickets/export')) ?>" formnovalidate>⤓ Export CSV</button>
  <a class="btn btn-ghost" href="<?= e(url('backoffice/tickets')) ?>" id="tkClear">Reset filtre</a>
</form>
<div class="card pad">
