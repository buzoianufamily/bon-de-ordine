...
$pageUrl = fn($p) => url('backoffice/audit').'?'.($qs?$qs.'&':'').'p='.$p;
?>
...
<form method="get" action="<?= e(url('backoffice/audit')) ?>" class="card pad" style="display:flex;gap:.8rem;flex-wrap:wrap;align-items:flex-end;margin-bottom:1.2rem">
  ...
  <button class="btn" formaction="<?= e(url('backoffice/audit/export')) ?>" formnovalidate>⤓ Export CSV</button>
  <a class="btn btn-ghost" href="<?= e(url('backoffice/audit')) ?>">Reset</a>
</form>
<div class="card pad">
